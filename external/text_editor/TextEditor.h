...
class TextEditor {
 public:
  ...
  // Public member functions start with an uppercase character to be consistent
  // with Dear ImGui
  //
  ...
  inline void ReplaceSectionText(int startLine,
                                 int startColumn,
                                 int endLine,
                                 int endColumn,
    ...
    return replaceSectionText(
        document.normalizeCoordinate(Coordinate(startLine, startColumn)),
        document.normalizeCoordinate(Coordinate(endLine, endColumn)), text);
  }
  ...
  inline void Render(const char* title,
  ...
  inline void SelectRegion(int startLine,
                           int startColumn,
                           int endLine,
                           int endColumn) {
  ...
  // get cursor positions (the meaning of main and current is explained in
  // README.md)
  ...
  inline void GetCursor(int& startLine,
                        int& startColumn,
                        int& endLine,
                        int& endColumn,
                        size_t cursor) const {
  ...
  // alternative API for cursor and selection position using lightweight out
  // struct (line and column are zero-based)
  ...
  // scrolling support
  enum class Scroll { alignTop, alignMiddle, alignBottom };
  ...
  //
  // calling SetCursor or ScrollToLine has no effect until the next call to
  // Render this is because we can only do layout calculations when we are in a
  // Dear ImGui drawing context as a result, SetCursor or ScrollToLine just mark
  // the request and let Render execute it
  //
  // the order of the calls is therefore important as they can interfere with
  // each other so if you call SetText, SetCursor and/or ScrollToLine before
  // Render, the order should be:
  //
  // * call SetText first as it resets the entire editor state including cursors
  // and scrolling
  // * then call SetCursor as it sets the cursor and requests that we make the
  // cursor visible (i.e. scroll to it)
  // * then call ScrollToLine to mark the exact scroll location (it cancels the
  // possible SetCursor scroll request)
  ...
  // specify a change callback (called when changes are made (including
  // undo/redo)) the delay parameter specifies a time in miliseconds that the
  // editor will wait for before calling which helps in case you don't need to
  // track every keystroke passing nullptr deactivates the callback
  ...
  // this callback is different from the one above as is reports every change
  // (not just a summary) and is very detailed the insert flag states whether
  // the change was an insert (true) or a delete (false) in case of an
  // overwrite, there will be two actions (first a delete and then an insert)
  // the start parameters refer to the insert point or the start of the delete
  // the end parameters refer to the end of the inserted text or the end of the
  // deleted text the text parameter contains the inserted or deleted text line,
  // column and index values are zero-based
  struct Change {
    bool insert;
    int startLine;
    int startColumn;
  ...
  // it provides a list of changes made to the document in a single transaction
  // (in the right order) be carefull with this callback as it gets very verbose
  // (called on every keystroke, delete, cut, paste, undo and redo) passing
  // nullptr deactivates the callback
  inline void SetTransactionCallback(
      std::function<void(std::vector<Change>&)> callback) {
    transactionCallback = callback;
  }

  // line-based callbacks (line numbers are zero-based)
  // insertor callback is called when for each line inserted and the result is
  // used as the new line specific user data deletor callback is called for each
  // line deleted (line specific user data is passed to callback) setting either
  // callback to nullptr will deactivate that callback
  ...
  // allowing integrators to associate external data with select lines or all
  // lines user data is an opaque void* that must be managed externally user
  // data is also passed to the decorator callback (see below) user data is
  // attached to a line and additions/insertions/deletions don't effect this if
  // a line with user data is removed, it won't come back on a redo the deletor
  // callback (if specified) is called when a line is deleted (see above)
  ...
  inline void SetLineDecorator(
      float width,
      std::function<void(Decorator& decorator)> callback) {
  ...
  // setup context menu callbacks (these are called when a user right clicks
  // line numbers or somewhere in the text) the editor sets up the popup menus,
  // the callback has to populate them
  inline void SetLineNumberContextMenuCallback(
      std::function<void(int line)> callback) {
  ...
  inline void SetTextContextMenuCallback(
      std::function<void(int line, int column)> callback) {
  ...
  // NOTE: functions provided to FilterSelections or FilterLines should accept
  ...
  class Language {
   public:
    ...
    // flag to describe if keywords and identifiers are case sensitive (which is
    // the default)
    bool caseSensitive = true;

    // the character that starts a preprocessor directive (can be 0 if language
    // doesn't have this feature)
    ImWchar preprocess = 0;

    // a character sequence that start a single line comment (can be blank if
    // language doesn't have this feature)
    std::string singleLineComment;

    // an alternate single line comment character sequence (can be blank if
    // language doesn't have this feature)
    std::string singleLineCommentAlt;

    // the start and end character sequence for multiline comments (can be blank
    // language doesn't have this feature)
    std::string commentStart;
    std::string commentEnd;

    // flags specifying whether language supports single quoted ['] and/or
    // double quoted [""] strings
    bool hasSingleQuotedStrings = false;
    bool hasDoubleQuotedStrings = false;

    // other character sequences that start and end strings (can be blank if
    // language doesn't have this feature)
    std::string otherStringStart;
    std::string otherStringEnd;

    // alternate character sequences that start and end strings (can be blank if
    // language doesn't have this feature)
    std::string otherStringAltStart;
    std::string otherStringAltEnd;

    // character inside string used to escape the next character (can be 0 if
    // language doesn't have this feature)
    ImWchar stringEscape = 0;

    // set of keywords, declarations, identifiers used in the language (can be
    // blank if language doesn't have these features) if language is not case
    // sensitive, all entries should be in lower case
    ...
    // functions to tokenize identifiers and numbers (can be nullptr if language
    // doesn't have this feature) start and end refer to the characters being
    // tokenized functions should return an iterator to the character after the
    // detected token returning start means no token was found
    ...
    // if a token is found, function should return an iterator to the character
    // after the token and set the color
    std::function<Iterator(Iterator start, Iterator end, Color& color)>
        customTokenizer;
  ...
  // iterate through identifiers detected by the colorizer (based on current
  // language)
  ...
  class AutoCompleteState {
   public:
    // current context (strings = UTF-8, columns = Nth visible column and
    // indices = Nth codepoint) to understand the difference between column and
    // index, think like a tab :-)
    ...
    // auto complete suggestions te be provided by app callback (the app is
    // responsible for sorting)

    // the editor does not automatically include language specific keywords or
    // identifiers in the suggestion list this is left to the application so it
    // can be context specific in case a language server is used a pointer to
    // the current language definition is provided so callbacks have easy access
    std::vector<std::string> suggestions;

    // set this to true if you are building the suggestion list asynchronously
    // and provide it later this way autocomplete is not cancelled if the
    // suggestion list is empty and the user hits tab or enter
  ...
  class AutoCompleteConfig {
   public:
    ...
    // specifies whether typing (or shortcut) in comments or strings triggers
    // autocomplete
    bool triggerInComments = false;
    bool triggerInStrings = false;

    // manual trigger key sequence (default is Ctrl+space on all platforms, even
    // MacOS) remember Dear ImGui reverses Ctrl and Command on MacOS
#if __APPLE__
    ...
    // text label used when no suggestions are available (this allows for
    // internationalization)
    std::string noSuggestionsLabel = "No suggestions";

    // called when autocomplete is configured, active and the editor needs an
    // updated suggestions list callback must populate and order suggestions in
    // state object suggestion list is not cleared by editor between callbacks
    // callback is called during the rendering loop (so don't take too long)

    // if it takes too long, applications should do search in separate thread
    // and use API to report results (see SetAutoCompleteSuggestions) callback
    // should set suggestionsPromise to true in this case
  ...
  // provide autocomplete suggestions asynchronously (in case a callback takes
  // to long and lookup is handled in a separate thread/process) this call is
  // not threadsafe and must be called from the rendering thread (you must
  // synchronize with your lookup thread yourself)
  inline void SetAutoCompleteSuggestions(
      const std::vector<std::string>& suggestions) {
    autocomplete.setSuggestions(suggestions);
  }

  // utility class to support some autocomplete implementations
  // this is not used by default but can be used in autocomplete callbacks (see
  // example app)
  class Trie {
   public:
    // constructor
    ...
    // populate list of suggestions based on provided search term (which is
    // UTF-8 encoded) limit is maximum number of suggestions returned after they
    // are sorted by relevance maxSkippedLetters is a the largest number of
    // letters that can be skipped to find the next match this allows for
    // missing letters (out of order letters are not taken into account)
  ...
  class CodePoint {
   public:
    static std::string_view::const_iterator skipBOM(
        std::string_view::const_iterator i,
        std::string_view::const_iterator end);
    static std::string_view::const_iterator read(
        std::string_view::const_iterator i,
        std::string_view::const_iterator end,
        ImWchar* codepoint);
    static size_t write(
        char* i,
        ImWchar codepoint);  // must point to buffer of 4 characters (returns
    ...
    static inline bool isPairOpener(ImWchar ch) {
      return ch == openCurlyBracket || ch == openSquareBracket ||
             ch == openParenthesis || ch == singleQuote || ch == doubleQuote;
    }

    static inline bool isPairCloser(ImWchar ch) {
      return ch == closeCurlyBracket || ch == closeSquareBracket ||
             ch == closeParenthesis || ch == singleQuote || ch == doubleQuote;
    }
 ...
 protected:
  ...
  class Coordinate {
    // represent a character coordinate from the user's point of view, i. e.
    // consider an uniform grid on the screen as it is rendered, and each cell
    // has its own coordinate, starting from 0
    //
    // tabs are counted as [1..tabsize] count spaces, depending on how many
    // spaces are necessary to reach the next tab stop
    //
    // for example, coordinate (1, 5) represents the character 'B' in a line
    // "\tABC", when tabsize = 4, because it is rendered as "    ABC" on the
    // screen
  ...
  class Cursor {
   public:
    ...
    // (these functions assume that insert or delete points are before the
    // cursor)
  ...
  class Cursors : public std::vector<Cursor> {
   public:
    ...
    // (these functions assume that insert or delete points are before the
    // cursor)
    void adjustForInsert(iterator start,
                         Coordinate insertStart,
                         Coordinate insertEnd);
    void adjustForDelete(iterator start,
                         Coordinate deleteStart,
                         Coordinate deleteEnd);
  ...
  class Document : public std::vector<Line> {
   public:
    ...
    Coordinate findPreviousNonWhiteSpace(Coordinate from,
                                         bool includeEndOfLine = true) const;
    Coordinate findNextNonWhiteSpace(Coordinate from,
                                     bool includeEndOfLine = true) const;
  ...
  // single action to be performed on the document as part of a larger
  // transaction
  class Action {
   public:
    // action types
    enum class Type : char { insertText, deleteText };

    // constructors
    Action() = default;
    Action(Type t, Coordinate s, Coordinate e, const std::string_view& txt)
        : type(t), start(s), end(e), text(txt) {}
  ...
  class Colorizer {
   ...
   private:
    ...
    bool matches(Line::iterator start,
                 Line::iterator end,
                 const std::string_view& text);

    // set color for specified range of glyphs
    inline void setColor(Line::iterator start,
                         Line::iterator end,
                         Color color) {
      while (start < end)
        (start++)->color = color;
  ...
  class BracketPair {
   public:
    BracketPair(ImWchar sc, Coordinate s, ImWchar ec, Coordinate e, int l)
        : startChar(sc), start(s), endChar(ec), end(e), level(l) {}
    ImWchar startChar;
    Coordinate start;
    ImWchar endChar;
    Coordinate end;
  ...
  class Bracketeer : public std::vector<BracketPair> {
   public:
    ...
    // update the list of bracket pairs in the document and colorize the
    // relevant glyphs
  ...
  class Autocomplete {
   public:
    ...
    // request autocomplete mode based on triggers (and if allowed by current
    // state) return value indicates whether autocomplete was initiated
    ...
    inline std::string getReplacement() {
      return currentSelection < state.suggestions.size()
                 ? state.suggestions[currentSelection]
                 : "";
    }
  ...
  // find/replace support
  void selectFirstOccurrenceOf(const std::string_view& text,
                               bool caseSensitive,
                               bool wholeWord);
  void selectNextOccurrenceOf(const std::string_view& text,
                              bool caseSensitive,
                              bool wholeWord);
  void selectAllOccurrencesOf(const std::string_view& text,
                              bool caseSensitive,
                              bool wholeWord);
  ...
  // transform selections (filter function should accept and return UTF-8
  // encoded strings)
  void filterSelections(std::function<std::string(std::string_view)> filter);
  void selectionToLowerCase();
  void selectionToUpperCase();

  // transform entire document (filter function should accept and return UTF-8
  // encoded strings)
  ...
  Coordinate insertText(std::shared_ptr<Transaction> transaction,
                        Coordinate start,
                        const std::string_view& text);
  void deleteText(std::shared_ptr<Transaction> transaction,
                  Coordinate start,
                  Coordinate end);
